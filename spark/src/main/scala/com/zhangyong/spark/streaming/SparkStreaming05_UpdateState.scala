...
object SparkStreaming05_UpdateState {
  def main(args: Array[String]): Unit = {
    //spark配置对象
    val sparkConf = new SparkConf().setMaster("local[*]").setAppName("SparkStreaming05_UpdateState")
    ...
    //保存数据状态，需要设定检查路径
    streamingContext.sparkContext.setCheckpointDir("spark/cp")
    ...
    val stateDStream = mapDStream.updateStateByKey {
      case (seq, buffer) => {
